...
export class Translations {
     ...
     * should be returned
     */
    public translateView(view: JQuery<HTMLElement>, doNotClone = false): JQuery<HTMLElement> {
    ...
    public translateTags($tags: JQuery<HTMLElement>) {
        ...
        for (const translatedTag of $translatedTags.toArray()) {
            ...
            const html = <string>table[translationId];
            if (html) {
                $t.html(html);
            }
        }
     ...
     */
    public text(textId: string, replacements: unknown[] = null): string {
        try {
            const table = this["texts"];
